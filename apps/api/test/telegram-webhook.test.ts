 ...
 *
 * ВНИМАНИЕ: здесь TELEGRAM_MODE=off, поэтому бот не создаётся вовсе и
 * handleUpdate вырождается в this.bot?.handleUpdate(...) — то есть в пустое
 * место. Проверяется только защита секретом, но НЕ обработка апдейта: эти
 * тесты зеленели, пока в production каждое сообщение падало с 500, потому что
 * grammY не был инициализирован. Настоящую обработку так не проверить —
 * init() ходит в сеть, а в тестах сеть перехвачена.
 */
